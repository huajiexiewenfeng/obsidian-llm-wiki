...
实施分支基于 `main` 的 `0cb31a5`。该基线已包含 runtime 权威源码目录、v0.2 Phase 2/3、
Phase 3.1 archive-import 和 Phase 4 Doctor 一致性检查；本机已安装 skill 缓存仍不作为实现来源。
根目录脚本继续只做兼容转发，不能维护两份权威实现。

用户已确认本规格并要求将 Inventory 作为当前最高优先级主线。实施测试顺序、活动文件范围和
隔离分支记录在同 Flow 的 working-context 与实施计划中。
