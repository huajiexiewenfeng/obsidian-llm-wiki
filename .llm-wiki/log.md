...
- Implemented Phase 3.1 archive import on `codex/v02-phase31-archive-import`: immutable Core-derived raw targets, lock-free streaming staging, no-replace publication, recovery, Doctor checks, CLI/E2E, and public workflow docs. Agent-local full verification passed 229 tests with 2 platform/opt-in skips; integration remains pending.
- Fast-forwarded Phase 3.1 into local `main@bb779eb`; the merged tree again passed 229 tests with 2 conditional skips. Remote push remains pending, and unrelated user changes in the main worktree were preserved.
- Pushed Phase 3.1 through `main@80d3846`, installed the project-level Skill set, and found a real-trial Doctor false positive for Core-generated control-center-relative Wikilinks. Fixed and pushed it as `cd31cc3`; the full suite passed 230 tests with 2 conditional skips, and installed-runtime validation reported matching archive bytes and zero Doctor ERROR findings.
- Completed v0.3 Inventory on `codex/v03-inventory`: Vault metadata discovery, baseline/registry comparison, confirmed Inventory mutations, Doctor findings/scoring/grouping, runtime packaging, and installed-skill trials. Agent-local verification passed 257 tests with 3 platform skips; the user requested a local fast-forward merge into `main`.
